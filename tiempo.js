...
const ElSalvadorModule = (function () {
  ...
  const weatherIconMap = {
    // ... (íconos de clima) const weatherIconMap = {
    "01d": "fas fa-sun",
    "01n": "fas fa-moon",
  ...
  async function updateWeatherInfo() {
    ...
    function actualizarIconoDiaNoche() {
      const ahora = new Date();
      const hora = ahora.getHours();
  
      if (hora >= 6 && hora < 18) {
        climaCard.classList.remove("noche");
        climaCard.classList.add("dia");
        iconoDia.style.display = "inline";
        iconoNoche.style.display = "none";
      } else {
        climaCard.classList.remove("dia");
        climaCard.classList.add("noche");
        iconoDia.style.display = "none";
        iconoNoche.style.display = "inline";
      }
    }
    if (data) {
